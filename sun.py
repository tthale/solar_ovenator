...
def get_inputs(args):
	...
	if (args.latitude == None or args.longitude == None):
		...
		print('Using default location:', 'Lat:', LA, 'Lon:', LO)
	else:
		LA = float(args.latitude)
		LO = float(args.longitude)
...
def sun_coords(date_time_local, LA, LO):
	...
	TLL = PI/180.0*LO + 2*PI/tday*t + PI #Theta of the latitude longitude coordinates in the earth's reference frame
	PLL = PI/2 - 2*PI/360.0*LA #Phi of the Latitude Longitude in the earth's reference frame
...
#code from imported sun_coords file
...
def main(args):
	...
	date_time_local, LA, LO = get_inputs(args)
...
"""
Run as python sun.py -d `date "+%Y-%m-%d %T"` 
might need to seperate the 'date "+%Y=%m-%d %T" into a separate temp shell thingy
"""
if __name__ == '__main__':
	parser = argparse.ArgumentParser()
	date_str=os.popen('date "+%Y-%m-%d %T"')
	date_str = date_str.read().strip()
	parser.add_argument('-d', '--date_time', default=None)
	parser.add_argument('-la', '--latitude', default=None)
	parser.add_argument('-lo', '--longitude', default=None)
